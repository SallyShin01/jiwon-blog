...
* 1 성별, 섬별 부리길이 평균계산
```{python}
bill_length = penguins.groupby(["sex","island"], as_index = False)['bill_length_mm'].mean(numeric_only=True)
```

* 2. 성별, 섬별 부리깊이 평균계산
```{python}
bill_depth = penguins.groupby(['sex','island'], as_index = False)['bill_depth_mm'].mean(numeric_only=True)
```
